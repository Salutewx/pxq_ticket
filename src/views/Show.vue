  ...
  <div class="p-4">
    ...
    <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 ">
      <div class="bg-white p-6 rounded-md shadow-md">
        ...
        <div class="p-4">
          <h4>票档:</h4>
          <select v-model="selectedSeatPlan" class="select select-success w-full max-w-xs">
            <option v-for="item in seatPlans" :key="item.seatPlanId" :value="item.seatPlanId">{{ item.seatPlanName }}
            </option>
          </select>
        </div>
...
interface QueryShowSessionResult {
  statusCode: number,
  data: Session[]
}
...
const updateSeatPlans = async () => {
  ...
  selectedSeatPlan.value = seatPlans.value[0].seatPlanId
}
...
const ticketWaitlist = async () => {
  const res: ticketWaitlistRes = await invoke("ticket_waitlist", { showId: selectedConcert.value.showId, 
    sessionId: sessions.value[selectedSessionIndex.value].bizShowSessionId,
  seatPlanId: selectedSeatPlan.value})
  if (res.statusCode == 200 && res.data.subscribed) {
